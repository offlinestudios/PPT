...
for(const key of Object.keys(D.svcData).filter(k=>k!=='document-services')){
 ...
 assert(d.querySelector('#sample-image').src.endsWith('/images/passport-scan-sample-square.jpg'));
 for(const option of d.querySelectorAll('#document option')){const control=d.querySelector('#document');control.value=option.value;control.dispatchEvent(new w.Event('change'));for(const button of d.querySelectorAll('[data-format]')){button.click();assert(d.querySelector('#sample-image').src.endsWith('/images/passport-scan-sample-square.jpg'),file+' shared scan');}}
